// Equipo 15: 319056323, 319302149, 422022569
// Proyecto Final
...
int main() {
    ...
    if (ma_engine_init(NULL, &gAudio) != MA_SUCCESS) {
        ...
        if (r != MA_SUCCESS) {
            std::cerr << "No se pudo cargar musica: " << musicPath << "\n";
        }
        else {
            ma_sound_set_looping(&gMusic, MA_TRUE);
            ma_sound_set_volume(&gMusic, gMusicVol);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform1i(glGetUniformLocation(skyboxShader.Program, "SkyBox"), 5);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode) {
    ...
    // --- Controles de música ---
    if (key == GLFW_KEY_M && action == GLFW_PRESS) { // Mute/Unmute
